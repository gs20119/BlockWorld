...
public class Network {
	...
	public double forward(double[] x, double[] y) { // only forward ( not train )
		...
		for(int i=1; i<depth; i++) { // for all layers
			for(int j=1; j<=size[i+1]; j++) { // for each perceptron
				...
				if(i+1!=depth) layers[i+1][j] = relu(layers[i+1][j]); // then activate ( a[i+1][j] )
			}
	...
	private double Adam() { 
		...
		for(int i=1; i<depth; i++) for(int j=0; j<=size[i]; j++) for(int k=1; k<=size[i+1]; k++) {
			...
			Error += Math.abs(mDecay/(Math.sqrt(vDecay)+eps)); Count++;
		}return Error / Count;
	}
	
	public double backward(double[] x, double[] y) { // forward + train ( backprop )
		...
		for(int i=depth-1; i>1; i--) { // delta = J:z[i][j] 
			for(int j=1; j<=size[i]; j++) {
				...
				for(int k=1; k<=size[i+1]; k++)
					if(i+1==depth) delta[i][j] += delta[i+1][k]*weights[i][j][k];
					else delta[i][j] += delta[i+1][k]*weights[i][j][k]*drelu(layers[i][j]);
		...
		return Adam(); // Optimize parameters and return converge Error
	}
}
